...
def setup_cli(modules_dir: Path, name: str = "SubParsley", desc: str = "SubParsley - Extensible CLI Tool") -> Any:
    ...
    """
    return _build_parser(load_modules_recursive(modules_dir), name, desc)


def _build_parser(modules: List[Tuple[str, Any]], name: str, desc: str) -> Any:
    """ Turn loaded modules into a parser.

        Split out of setup_cli so directory scanning and package importing —
        two ways of FINDING modules — share one way of turning them into a CLI.
    """
    ...
    nouns: Dict[str, List[Tuple[str, Callable, Optional[str], Dict[str, Any]]]] = {}
...
PACKAGE_ENV_VAR = "PROJECT_PACKAGE"
VERSION_ENV_VAR = "PROJECT_VERSION"
REQUIRES_ENV_VAR = "PROJECT_REQUIRES_SUBPARSLEY"
...
def check_requirement(requirement: str, found: str = None, source: str = None):
    """ Enforce a consumer's declared SubParsley requirement.

        The CONSUMER declares (`PROJECT_REQUIRES_SUBPARSLEY=">=0.3,<0.4"`) and
        SubParsley checks, so every consumer gets the check for free and none
        reimplements version parsing.

        A deliberately tiny comparator: `>=X.Y[.Z]` with an optional `,<A.B[.C]`,
        and nothing else. Depending on `packaging` would cost this framework its
        only real distinguishing property — being one stdlib-only file you can
        copy into a project.

        SemVer here is over the GENERATED CLI SURFACE, not the Python API:
        renaming or rebinding a flag that a consumer's docstrings already
        produce is a breaking change, even though no function signature moved.
    """
    if not requirement:
        return
    found = found or __version__
    lower = upper = None
    for clause in (c.strip() for c in requirement.split(",")):
        if not clause:
            continue
        if clause.startswith(">="):
            lower = clause[2:].strip()
        elif clause.startswith("<"):
            upper = clause[1:].strip()
        else:
            fail(f"Unsupported {REQUIRES_ENV_VAR} clause {clause!r}: only "
                 f"'>=X.Y' and '<X.Y' are understood")
    for value in (v for v in (lower, upper) if v):
        if not _VERSION_RE.match(value):
            fail(f"Malformed version in {REQUIRES_ENV_VAR}: {value!r}")

    where = f" ({source})" if source else ""
    if lower and _version_tuple(found) < _version_tuple(lower):
        fail(f"SubParsley {found}{where} is too old: this project requires "
             f"{requirement}")
    if upper and _version_tuple(found) >= _version_tuple(upper):
        fail(f"SubParsley {found}{where} is too new: this project requires "
             f"{requirement}")
...
def resolve_config() -> Tuple[Path, str, str]:
    ...
    # `Path("") or fallback` never takes the fallback: Path("") is Path("."),
    # which is truthy, so an unset PROJECT_DIR silently meant the CWD.
    raw_dir = os.environ.get("PROJECT_DIR", "").strip()
    project_dir = Path(raw_dir) if raw_dir else Path(__file__).parent
...
def load_package_modules(package_name: str, ignore: Sequence[str] = None,
                         on_error: Callable[[str, BaseException], None] = None
                         ) -> List[Tuple[str, Any]]:
    """ Import every submodule of an installed package, by its real dotted name.

        The directory-scanning path (`load_modules_recursive`) has to put
        PROJECT_DIR on sys.path, which for an INSTALLED consumer means inserting
        its package directory — making `models`, `services`, `ext` importable as
        bare top-level names again, exactly the flat namespace that packaging it
        was meant to escape.

        Here nothing touches sys.path: the package is already importable, so its
        submodules are imported as `pkg.sbor`, `pkg.ext.finj.portfolio`, and the
        names they occupy are the ones they actually have.
    """
    if ignore is None:
        ignore = resolve_ignore_patterns()
    if on_error is None:
    ...
    try:
        ...
        fail(f"Cannot import {PACKAGE_ENV_VAR}={package_name!r}: {exc}", exc)
        return []

    root = Path(next(iter(package.__path__)))
    modules: List[Tuple[str, Any]] = []

    def walk(directory: Path, dotted: str):
        for module_file in sorted(directory.glob("*.py")):
            if module_file.name == "__init__.py" or _is_ignored(module_file, root, ignore):
                continue
            name = f"{dotted}.{module_file.stem}"
            try:
        ...
        for subdir in sorted(directory.iterdir()):
            if (subdir.is_dir() and not subdir.name.startswith("_")
                    and not _is_ignored(subdir, root, ignore)
                    and (subdir / "__init__.py").exists()):
                walk(subdir, f"{dotted}.{subdir.name}")

    walk(root, package_name)
    return modules


def setup_cli_from_package(package_name: str, name: str = "SubParsley",
                           desc: str = "SubParsley - Extensible CLI Tool") -> Any:
    """setup_cli, but resolving dispatchers through an importable package."""
    return _build_parser(load_package_modules(package_name), name, desc)


def main(argv: List[str] = None):
    project_dir, project_name, project_desc = resolve_config()
    package = os.environ.get(PACKAGE_ENV_VAR, "").strip()

    check_requirement(os.environ.get(REQUIRES_ENV_VAR, "").strip(),
                      source=str(Path(__file__).resolve()))

    # Handled before the parser is built: `--version` must work even if a
    # consumer module fails to import, which is when you most want to know
    # which SubParsley you are running.
    if argv is None:
        argv = sys.argv[1:]
    if "--version" in argv:
        consumer = os.environ.get(VERSION_ENV_VAR, "").strip()
        shown = f"{project_name} {consumer} " if consumer else f"{project_name} "
        print(f"{shown}(SubParsley {__version__} at {Path(__file__).resolve()})")
        return 0

    if package:
        parser = setup_cli_from_package(package, project_name, project_desc)
    else:
